...
agent_tools = [execute_bash, read_file, write_file, append_file, send_message, check_messages, wait_for_reply]
...
# Prompt 定義
planning_prompt = """你是一個專業的任務拆解與系統規劃專家。請將任務解構為多個子任務，並指派專屬 AI Agent。
請注意：這些 Agent 將會「平行且同時」執行。為確保他們能無縫協作，請針對會產生依賴的職責（例如前端依賴後端API）做好角色劃分，
他們將配備訊息通訊工具來解決依賴問題。
